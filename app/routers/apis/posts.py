...
def gets_posts(current_user:int =Depends(get_current_user),
db:Session = Depends(get_db),limit:int = 10,skip:int =0,title:Optional[str] = ''):
    
    posts = db.query(Post).filter(Post.owner_id == current_user.id).filter(Post.title.contains(title)).limit(limit).offset(skip).all()
    print(posts)
...
async def gets_post(id:int,db : Session=Depends(get_db),current_user:int = Depends(get_current_user),title:Optional[str]=""):
    
    #cursor.execute("""SELECT * FROM posts where id=(%s)""",(str(id)))
    #post = cursor.fetchone()
    post = db.query(Post).filter(Post.id==id).filter(Post.title.contains(title)).first()
...
async def gets_posts(db: Session = Depends(get_db),current_user:int=Depends(get_current_user),limit:int = 10,skip:int =0,
title:Optional[str] = ''):
    
    posts =  db.query(Post).filter(Post.published==True).filter(Post.title.contains(title)).offset(skip).limit(limit).all()
...
async def create_post(new_post : Post_create, db:Session = Depends(get_db),
...
def update(id:int,update_post: Post_create, db:Session=Depends(get_db),
current_user: int = Depends(get_current_user)):
